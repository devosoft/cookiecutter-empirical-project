...
COPY . /opt/cookiecutter-empirical-project

RUN \
  pip3 install -r /opt/cookiecutter-empirical-project/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/cookiecutter-empirical-project \
    && \
  chmod --recursive g+rwx /opt/cookiecutter-empirical-project \
    && \
  echo "user granted permissions to /opt/cookiecutter-empirical-project"

USER user

RUN \
  cd /opt/cookiecutter-empirical-project \
    && \
...
WORKDIR /opt/cookiecutter-empirical-project
